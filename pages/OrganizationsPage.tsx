...
const OrganizationsPage = () => {
  ...
  const fetchStudentsQuery = OrganizationService.fetchOrganizationsServiceQuery(
    {
      page: currentPage || 1,
  ...
  return (
    <div className='space-y-6 animate-fade-in w-full max-w-full overflow-x-auto'>
      ...
      <div className='flex flex-col md:flex-row justify-between items-start md:items-end gap-4'>
        <div>
          <h2 className='text-2xl font-bold text-brand-dark'>Organizations</h2>
        </div>
      </div>
      {/* Filters */}
        ...
        <div className='flex-1 relative'>
          ...
          >
            <div className='flex items-center'>
              <input
                type='text'
                placeholder='Search by organization name or email or industry of size...'
        ...
        <div className='overflow-x-auto'>
          <table className='w-full text-left min-w-[800px]'>
            ...
            <tbody className='divide-y divide-slate-100'>
              {(fetchStudentsQuery?.data?.data || []).map((organization) => (
                <OrganizationItem
                  page={currentPage}
